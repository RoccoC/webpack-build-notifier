...
export default class WebpackBuildNotifierPlugin {
  ...
  private readonly onCompilationDone = (results: webpack.Stats): void => {
    ...
    if (this.getWarningOrErrorCount(results.compilation, 'errors') >= 1) {
      ...
      !this.suppressWarning
      && this.getWarningOrErrorCount(results.compilation, 'warnings') >= 1
    ) {
      const warning = this.getFirstWarningOrError(
        results.compilation,
        'warnings',
      );
      const warningFilePath = warning.module && warning.module.resource
    ...
    const notifyOptions = (typeof this.notifyOptions === 'function'
